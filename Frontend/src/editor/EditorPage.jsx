...
const EditorPage = () => {
  ...
  useEffect(() => {
    inlineSuggestEnabledRef.current = inlineSuggestEnabled;
  ...
  const fetchInlineSuggestion = async (codeBefore, codeAfter, language) => {
    try {
      const res = await axiosInstance.post(
        "https://real-time-code-editor-vercodex.onrender.com/vercodex/code/inlineSuggestion",
  ...
  return (
    <div className="h-screen bg-[#1e1e1e] flex flex-col overflow-hidden">
      ...
      <div className="h-12 border-b border-gray-700 flex items-center px-4 gap-2 relative">
        ...
        [&_pre]:bg-black [&_pre]:rounded [&_pre]:p-2 [&_pre]:overflow-x-auto
        [&_code]:bg-black [&_code]:px-1 [&_code]:rounded [&_code]:text-green-400
        [&_pre_code]:bg-transparent [&_pre_code]:p-0
        [&_ul]:list-disc [&_ul]:ml-4 [&_ol]:list-decimal [&_ol]:ml-4
        [&_li]:my-0.5 [&_p]:my-1 [&_strong]:text-white [&_h1]:text-base
        [&_h2]:text-sm [&_h3]:text-sm [&_blockquote]:border-l-2
        [&_blockquote]:border-gray-500 [&_blockquote]:pl-2 [&_blockquote]:text-gray-400"
                          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {selectedFile ? (
              <Editor
                ...
                onMount={(editor, monacoInstance) => {
                  ...
                  // Guard against re-registering on every file switch (the Editor remounts
                  // because of the `key` prop, but this ref persists across remounts)
                  if (!inlineProviderRegisteredRef.current) {
                    inlineProviderRegisteredRef.current = true;

                    let debounceTimer = null;
                    let lastFetchKey = null;
                    let lastFetchSuggestion = "";

                    monacoInstance.languages.registerInlineCompletionsProvider(
                      SUPPORTED_INLINE_LANGUAGES,
                      {
                        provideInlineCompletions: (
                          ...
                          if (!inlineSuggestEnabledRef.current) {
                            return { items: [] };
                          }
                          ...
                          // Unique fingerprint of the current code + cursor state.
                          const fetchKey = `${language}::${codeBefore}::${codeAfter}`;

                          // If nothing actually changed since the last fetch (e.g. Monaco
                          // re-invoking this during idle time), reuse the cached result —
                          // no debounce timer, no API call.
                          if (fetchKey === lastFetchKey) {
                            return {
                              items: lastFetchSuggestion
                                ? [
                                    {
                                      insertText: lastFetchSuggestion,
                          ...
                          return new Promise((resolve) => {
                            ...
                            debounceTimer = setTimeout(async () => {
                              ...
                              // Cache it regardless of outcome, so a repeat call with the
                              // same key (e.g. during idle) won't re-fetch.
                              lastFetchKey = fetchKey;
                              lastFetchSuggestion = suggestion;
                            ...
                            }, 3000); // wait 3s of inactivity before firing
          ...
          >
            <div className="h-10 border-b border-gray-700 flex items-center justify-between px-3 text-white font-semibold">
              <span>Terminal</span>
              <button
              ...
              >
                Inline: {inlineSuggestEnabled ? "On" : "Off"}
              </button>
            </div>
            <div className="flex-1 flex flex-col overflow-hidden">
