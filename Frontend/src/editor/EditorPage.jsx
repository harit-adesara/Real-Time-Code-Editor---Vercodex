  ...
  <ReactMarkdown
    components={{
      ...
      code({ node, className, children, ...props }) {
        const isInline = !className; // fenced blocks have "language-xxx"
        return isInline ? (
          <code
            className="bg-black/40 text-cyan-300 px-1 py-0.5 rounded text-xs font-mono"
            {...props}
      ...
      ul({ children }) {
        return (
          <ul className="list-disc ml-4 space-y-0.5 text-gray-300 my-1">
            {children}
          </ul>
        );
      },
      ol({ children }) {
        return (
          <ol className="list-decimal ml-4 space-y-0.5 text-gray-300 my-1">
            {children}
          </ol>
        );
      },
      ...
      blockquote({ children }) {
        return (
          <blockquote className="border-l-2 border-cyan-500 pl-3 my-2 text-gray-400 italic">
            {children}
          </blockquote>
        );
      },
...
const EditorPage = () => {
  ...
  const sendMessageToAI = async () => {
    ...
    try {
      ...
      // ── LIVE STREAM: read chunks as they arrive ──────────────────────────
      const reader = res.body.getReader();
      const decoder = new TextDecoder("utf-8");
      let aiText = "";
      ...
      while (true) {
        ...
        // decode may return partial multi-byte chars; stream:true handles that
        const chunk = decoder.decode(value, { stream: true });
        aiText += chunk;

        // Update only the last message (the AI placeholder) in-place
        setChatMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = {
            ...updated[updated.length - 1],
              ...
              <div className="flex-1 overflow-y-auto px-3 py-2 space-y-3 text-sm">
                ...
                {chatMessages.map((msg, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`px-3 py-2 rounded-lg max-w-[90%] text-sm break-words ${
                        msg.role === "user"
                          ? "bg-cyan-700 text-white"
                          : "bg-[#2d2d2d] text-gray-200"
                      }`}
                    >
                      {msg.role === "user" ? (
                        ...
                        // AI placeholder while streaming hasn't started
